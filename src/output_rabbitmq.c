...
static void R_API_CALLCONV print_rabbitmq_data(data_output_t *output, data_t *data, char const *format)
{
    ...
    if (data_model || data_mod) {
        ...
        if (data_model) {
            // This is decoded device data - create combined message with pulse info
            data_print_jsons(data, json_buffer, sizeof(json_buffer) - 500);  // Leave space for pulse info
            
            // Look for pulse and frequency data in the decoded message
            data_t *data_freq = NULL;
            data_t *data_rssi = NULL;
            data_t *data_snr = NULL;
            for (data_t *d = data; d; d = d->next) {
                if (!strcmp(d->key, "freq"))
                    data_freq = d;
                if (!strcmp(d->key, "rssi"))
                    data_rssi = d;
                if (!strcmp(d->key, "snr"))
                    data_snr = d;
            }
            
            // Remove the closing brace to add pulse info
            size_t len = strlen(json_buffer);
            if (len > 0 && json_buffer[len-1] == '}') {
                json_buffer[len-1] = '\0';  // Remove closing brace
                
                // Add pulse analysis info to the message
                char pulse_info[500];
                char freq_str[32] = "433920000";
                char rssi_str[32] = "-20.0";
                char snr_str[32] = "10.0";
                
                // Extract values based on their type
                if (data_freq) {
                    if (data_freq->type == DATA_INT) {
                        snprintf(freq_str, sizeof(freq_str), "%d", data_freq->value.v_int);
                    } else if (data_freq->type == DATA_DOUBLE) {
                        snprintf(freq_str, sizeof(freq_str), "%.0f", data_freq->value.v_dbl);
                    } else if (data_freq->type == DATA_STRING && data_freq->value.v_ptr) {
                        strncpy(freq_str, (char*)data_freq->value.v_ptr, sizeof(freq_str)-1);
                        freq_str[sizeof(freq_str)-1] = '\0';
                    }
                }
                
                if (data_rssi) {
                    if (data_rssi->type == DATA_DOUBLE) {
                        snprintf(rssi_str, sizeof(rssi_str), "%.1f", data_rssi->value.v_dbl);
                    } else if (data_rssi->type == DATA_STRING && data_rssi->value.v_ptr) {
                        strncpy(rssi_str, (char*)data_rssi->value.v_ptr, sizeof(rssi_str)-1);
                        rssi_str[sizeof(rssi_str)-1] = '\0';
                    }
                }
                
                if (data_snr) {
                    if (data_snr->type == DATA_DOUBLE) {
                        snprintf(snr_str, sizeof(snr_str), "%.1f", data_snr->value.v_dbl);
                    } else if (data_snr->type == DATA_STRING && data_snr->value.v_ptr) {
                        strncpy(snr_str, (char*)data_snr->value.v_ptr, sizeof(snr_str)-1);
                        snr_str[sizeof(snr_str)-1] = '\0';
                    }
                }
                
                snprintf(pulse_info, sizeof(pulse_info), 
                    ",\"pulse_analysis\":{\"signal_detected\":true,\"modulation\":\"detected_by_decoder\",\"freq_hz\":%s,\"rssi_db\":%s,\"snr_db\":%s,\"analysis\":\"Combined raw and decoded data for debugging\"}}",
                    freq_str, rssi_str, snr_str
                );
                
                strcat(json_buffer, pulse_info);
            }
            
            target_queue = rabbitmq->detected_queue;
            print_logf(LOG_DEBUG, "RabbitMQ", "Sending combined decoded+pulse to 'detected': %.100s...", json_buffer);
